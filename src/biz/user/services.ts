import { request1 } from "@/biz/requests";
...
export function logout(body: { email: string; password: string }) {
  return request1.post("/api/admin/user/logout", body);
}

export function get_token() {
  return request1.post("/api/token", {});
}
...
export function fetch_user_profile() {
  return request1.get("/api/admin/user/profile");
}
...
export function validate(token: string) {
  return request1.post<{ token: string }>("/api/admin/user/validate", { token });
}
