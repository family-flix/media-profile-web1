...
export function createSyncTaskWithUrl(body: {
  ...
  return request1.post<{}>("/api/v2/admin/sync_task/create", {
    url,
    pwd,
    resource_file_id,
    resource_file_name,
...
export function runSyncTask(body: { id: string }) {
  const { id } = body;
  return request1.post<{ job_id: string }>("/api/v2/admin/sync_task/run", {
    id,
  });
}
/**
...
export function completeSyncTask(params: { id: string }) {
  const { id } = params;
  return request1.post<void>("/api/v2/admin/sync_task/complete", { id });
}
/**
 * 删除同步任务
 */
export function deleteSyncTask(params: { id: string }) {
  const { id } = params;
  return request1.post<void>("/api/v2/admin/sync_task/delete", { id });
}
/**
 * 给指定同步任务覆盖另一个分享资源
 */
export function overrideResourceForSyncTask(values: {
  ...
  return request1.post<void>("/api/v2/admin/sync_task/override", {
    id,
    url,
    pwd,
    resource_file_id,
