...
export function createMember(body: { remark: string }) {
  return request1.post<{ id: string; account: { id: string; pwd: string } }>("/api/v2/admin/member/add", body);
}
/**
 * 生成成员授权链接
 * @param body
...
export function createMemberAuthToken(body: { id: string }) {
  return request1.post<{ id: string }>("/api/v2/admin/member/add_token", body);
}
/**
 * 删除指定成员
 * @returns
...
export function fetchSettings() {
  return request1.post<UserSettings>("/api/v2/admin/settings/profile", {});
}
/**
 * 更新用户配置
 */
export function updateSettings(values: Partial<UserSettings>) {
  ...
  return request1.post("/api/v2/admin/settings/update", {
    third_douban,
    push_deer_token,
    telegram_token,
    extra_filename_rules,
...
export function pushMessageToMembers(values: { content: string }) {
  return request1.post("/api/admin/notify", values);
}
...
export function replyReport(
  ...
  return request1.post("/api/v2/admin/report/reply", {
    id: report_id,
    content,
    media_id,
  });
...
export function addPermission(values: { desc: string }) {
  ...
  return request1.post("/api/admin/permission/add", body);
}
/**
 * 校验字幕文件名是否合法
 */
...
export function refresh_media_ranks() {
  return request1.post("/api/v2/admin/collection/refresh_media_rank");
}
