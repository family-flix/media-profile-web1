...
export function setMediaProfile(body: {
  ...
  return request1.post<void>("/api/v2/admin/media/set_profile", {
    media_id,
    media_profile,
  });
}
...
export function fetchMediaListPrepareArchive(params: FetchParams & Partial<{ name: string; drive_ids: string }>) {
  const { page, pageSize, ...rest } = params;
  return request1.post<ListResponseWithCursor<MediaPrepareArchiveItemResp>>("/api/v2/admin/media/archive/list", {
    ...rest,
    page,
    page_size: pageSize,
  });
...
export function fetchPartialMediaPrepareArchive(body: { media_id: string }) {
  const { media_id } = body;
  return request1.post<MediaPrepareArchiveItemResp>("/api/v2/admin/media/archive/partial", {
    media_id,
  });
}
...
export function deleteMedia(body: { media_id: string }) {
  const { media_id } = body;
  return request1.post("/api/v2/admin/media/delete", {
    media_id,
  });
}
...
export function transferMediaToAnotherDrive(body: { media_id: string; to_drive_id: string }) {
  ...
  return request1.post<{ job_id: string }>("/api/v2/admin/media/transfer", {
    media_id,
    to_drive_id,
  });
}
...
export function transferMediaToResourceDrive(body: { media_id: string }) {
  const { media_id } = body;
  return request1.post<{ job_id: string }>("/api/v2/admin/media/to_resource_drive", {
    media_id,
  });
}
/**
...
}>("/api/v2/media_profile/profile", values);
}
export type MediaProfile = NonNullable<UnpackedResult<TmpRequestResp<typeof fetchMediaProfile>>>;
export type MediaEpisodeItem = NonNullable<
  UnpackedResult<TmpRequestResp<typeof fetchMediaProfile>>
>["episodes"][number];
export function fetchMediaFilePreview(values: { id: string }) {
  return request1.post<{
    url: string;
    thumbnail: string;
    type: string;
  ...
  }>("/api/v2/admin/parsed_media_source/preview", values);
}
