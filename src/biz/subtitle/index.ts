...
import { extraLanguageFromSubtitleURL, parseSubtitleContent, parseSubtitleURL, timeStrToSeconds } from "./utils";
...
export class SubtitleCore extends BaseDomain<TheTypesOfEvents> {
  static async New(
    subtitle: { id: string; type: SubtitleFileTypes; url: string; name: string },
    extra: { currentTime?: number; client: HttpClientCore }
  ) {
    const { id, name, type, url } = subtitle;
    const language = extraLanguageFromSubtitleURL(name);
    const { client } = extra;
    const content_res = await (async () => {
      ...
      if (type === SubtitleFileTypes.LocalFile) {
        try {
          const r = await client.fetch<string>({ url: ["", url].join(""), method: "GET" });
          return Result.Ok({
            name,
            content: r.data,
          });
    ...
    const store = new SubtitleCore({
      filename: subtitle_name.replace(`.${suffix}`, "").replace(language.join("&"), ""),
      language,
      suffix,
      lines: paragraphs,
    });
